...
class utils():
    ...
    def add_bbox_element(bbox,sz,padding=0):
        ...
        for i in range(0,len(bbox)):
            box.append(utils.add_part_element(bbox[i,:],i,sz))
        return box
    ...
    def add_image_element(image, coords, sz, path):
        ...
        image_e.append(utils.add_bbox_element(coords,sz))
        return image_e
    
    def generate_dlib_xml(images,sizes,folder,out_file):
        '''
        ...
        for i in range(0,len(images['im'])):
            ...
            if os.path.isfile(path) is True: 
                ...
                if path in present_tags:
                    ...
                    images_e.append(utils.add_image_element(name,images['coords'][i],sizes[name],path))
    ...
    def dlib_xml_to_pandas(xml_file: str, print_csv = False):
        ...
        df=df[utils.natural_sort_XY(df)]
        if print_csv:
